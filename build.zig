...
pub fn build(b: *std.Build) void {
    ...
    const platform_files = [_][]const u8{
        "src/i_main.c",
        "src/i_net.c",
        "src/i_sound.c",
        "src/i_system.c",
    ...
    const fmt_step = b.step("fmt", "Format platform layer (i_*.c/h) with clang-format");
    const fmt_run = b.addSystemCommand(&.{ "clang-format", "-i" });
    fmt_run.addArgs(&platform_files);
    fmt_step.dependOn(&fmt_run.step);

    const fmt_check_step = b.step("fmt-check", "Check platform layer formatting without modifying");
    const fmt_check_run = b.addSystemCommand(&.{ "clang-format", "--dry-run", "--Werror" });
    fmt_check_run.addArgs(&platform_files);
